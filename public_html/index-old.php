...
$now = date('y-m-d H:i');
$query = "SELECT * from news as n INNER JOIN categories as c ON c.id=n.category  where n.category='4' AND n.status='Published' AND n.pub_date_time < '$now' AND n.slider='Yes' GROUP BY n.newsid ORDER BY n.slider_priority ASC";
$bnews = $db_handle->runQuery($query);

$lnsql = "SELECT * from news as n INNER JOIN categories as c ON c.id=n.category where n.status='Published' AND n.pub_date_time <= '$now' AND n.newstype != 'Video' GROUP BY n.newsid order by n.newsid DESC";
...
WHERE pc.category='1' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.latest_priority ASC";
...
ON p.newsid = pc.news_id
JOIN categories AS c ON c.id=p.category
WHERE pc.category='11' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.latest_priority DESC LIMIT 2";
$gmnews = $db_handle->runQuery($gmqry);

$tnews1 = mysqli_query($con,"SELECT p.team_id,p.newsid,p.videoid,c.hindi_name, p.title, p.image, p.newsurl, p.date, p.time, p.metat, p.metad, p.metad FROM news p INNER JOIN news_cat pc ON p.newsid = pc.news_id INNER JOIN categories c ON p.category=c.id WHERE pc.category = '24' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.newsid DESC LIMIT 1");

$row=mysqli_fetch_array($tnews1);

$vnews="SELECT p.team_id,p.newsid,p.videoid,c.hindi_name, p.title, p.image, p.newsurl, p.date, p.time, p.metat, p.metad, p.metad FROM news p INNER JOIN news_cat pc ON p.newsid = pc.news_id INNER JOIN categories c ON p.category=c.id WHERE pc.category = '10' AND p.newsid!='".$row['newsid']."' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.newsid DESC LIMIT 3";

$enews="SELECT p.team_id,p.newsid,c.hindi_name, p.title, p.short_description, p.image, p.newsurl, p.date, p.time, p.metat, p.metad, p.metad FROM news p INNER JOIN news_cat pc ON p.newsid = pc.news_id INNER JOIN categories c ON p.category=c.id WHERE pc.category = '78' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.newsid DESC LIMIT 2";


$pnews="SELECT p.team_id,p.newsid,c.hindi_name, p.title, p.short_description, p.image, p.newsurl, p.date, p.time, p.metat, p.metad, p.metad FROM news p INNER JOIN news_cat pc ON p.newsid = pc.news_id INNER JOIN categories c ON p.category=c.id WHERE pc.category = '25' AND p.status='Published' AND p.pub_date_time <= '$now' GROUP BY p.newsid  order by p.newsid DESC LIMIT 5";
...
?>
				...
				<div class="col-lg-6">
					...
					<div class="ts-grid-box watch-post mb-2" style="padding: 0px;">
								...
								<div class="tab-content featured-post">
									<div class="tab-pane ts-overlay-style fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
										<div class="item" style="min-height: 318px;background-image: url(<?php echo $urlroot; ?>images/news/<?php echo $row["image"]; ?>)">

											<a class="post-cat ts-orange-bg" href="#"> <?php echo $row['hindi_name'];?></a>
											<a href="https://www.youtube.com/watch?v=<?php echo $row['videoid'];?>" class="ts-video-btn">
												<i class="fa fa-play-circle-o"></i>
											</a>
											<div class="overlay-post-content">
												<div class="post-content">
													<h3 class="post-title md">
														<a href="<?php echo $urlroot;?>news/<?php echo $row['newsurl'];?>"><?php echo $row['title'];?></a>
													</h3>
													<ul class="post-meta-info">
														<li class="author">
<?php
$auth2 = mysqli_query($con,"SELECT `name`,`image`,`email` FROM `team` WHERE `t_id`='".$row['team_id']."'");
$au2 = mysqli_fetch_array($auth2);
?>
															<a href="#">
																<img src="<?php echo $urlroot.'team/'.$au2['image']; ?>" alt="<?php echo $au2['name']; ?>"> <?php echo $au2['name']; ?>
															</a>
														</li>
														<!--<li>
															<i class="fa fa-clock-o"></i>
															<?php echo $pubDate;?> , <?php echo $row['time'];?>
														</li>-->
					...
					<div class="post-list-item" >
						...
						<ul class="nav nav-tabs" role="tablist">
							<li role="presentation">
								<a class="active" href="aggregator_more.php"  aria-controls="home">									
									और देखें
								</a>
							</li>
...
<?php
$qry="SELECT * FROM `rss_feed`  ORDER BY sequence DESC LIMIT 2";
$ex_rss=mysqli_query($con,$qry);
							$i=1; while($row=mysqli_fetch_array($ex_rss)) {
							$url=$row['link'];	
							$image_rss=$row['image_name'];	
        ...
        $i=0;
            ...
            $siteimage = $feeds->channel->image_name; 
		...
		<!---<div class="post-content media" >
												<div class="media-body">
														<img class="d-flex sidebar-img" src="<?php echo $image; ?>" alt="">
											
												<a style="font-size: 12px;font-weight: 600; letter-spacing: 1px;    line-height: 15px;" href="<?php echo $link; ?>" class="blue-color"> <?php echo $title; ?></a>
														 <span style="font-size: 12px;"><?php echo $pubDate; ?></span>
													</span>
													<h4 class="post-title">
														<a style="font-size: 10px;" href="<?php echo $link; ?>"><?php echo implode(' ', array_slice(explode(' ', $description), 0, 15)) . "..."; ?><span style="color:red;"> Read more </span></a>
													</h4>
												</div>
											</div>--->
											
				           <div class="post-content media">
								<img class="d-flex sidebar-img" src="<?php echo $urlroot.'images/gallery/'.$image_rss; ?>" alt="">
											
									<div class="media-body">
										<span class="post-tag">
											<a href="<?php echo $link; ?>" class="green-color"> </a>
										</span>
										<span class="post-tag">
											<a href="<?php echo $link; ?>" class="green-color"> <?php echo $pubDate; ?></a>
										</span>
										<h4 class="post-title">
											<a href="<?php echo $link; ?>"><?php echo implode(' ', array_slice(explode(' ', $title), 0, 15)) . "..."; ?></a>
										</h4>
									</div>
								</div>
